...
/****** Object:  StoredProcedure [dbo].[Daily_Alarm_Report_Plant_Site_FULL_INSTRUMENTATION]    Script Date: 20/09/2021 9:36:52 AM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[Daily_Alarm_Report_Plant_Site_FULL_INSTRUMENTATION]
AS
BEGIN
	EXEC('
...
SET @xml1 = CAST((SELECT [Priority] AS ''td'','''', [Timestamp] AS ''td'','''',[Source] AS ''td'','''',[Condition] AS ''td'','''',[Message] AS ''td'','''', [Occurence] AS ''td'','''' FROM  [DCS].dbo.Daily_Alarm_Events_AIRSEP_INSTRUMENTATION FOR XML PATH(''tr''), ELEMENTS) AS NVARCHAR(MAX))
SET @xml2 = CAST((SELECT [Priority] AS ''td'','''', [Timestamp] AS ''td'','''',[Source] AS ''td'','''',[Condition] AS ''td'','''',[Message] AS ''td'','''', [Occurence] AS ''td'','''' FROM  [DCS].dbo.Daily_Alarm_Events_HYDROGEN_INSTRUMENTATION FOR XML PATH(''tr''), ELEMENTS) AS NVARCHAR(MAX))
SET @xml3 = CAST((SELECT [Priority] AS ''td'','''', [Timestamp] AS ''td'','''',[Source] AS ''td'','''',[Condition] AS ''td'','''',[Message] AS ''td'','''', [Occurence] AS ''td'','''' FROM  [DCS].dbo.Daily_Alarm_Events_PAL_INSTRUMENTATION FOR XML PATH(''tr''), ELEMENTS) AS NVARCHAR(MAX))
SET @xml4 = CAST((SELECT [Priority] AS ''td'','''', [Timestamp] AS ''td'','''',[Source] AS ''td'','''',[Condition] AS ''td'','''',[Message] AS ''td'','''', [Occurence] AS ''td'','''' FROM  [DCS].dbo.Daily_Alarm_Events_POWER_INSTRUMENTATION FOR XML PATH(''tr''), ELEMENTS) AS NVARCHAR(MAX))
SET @xml5 = CAST((SELECT [Priority] AS ''td'','''', [Timestamp] AS ''td'','''',[Source] AS ''td'','''',[Condition] AS ''td'','''',[Message] AS ''td'','''', [Occurence] AS ''td'','''' FROM  [DCS].dbo.Daily_Alarm_Events_REFINERY_INSTRUMENTATION FOR XML PATH(''tr''), ELEMENTS) AS NVARCHAR(MAX))
SET @xml6 = CAST((SELECT [Priority] AS ''td'','''', [Timestamp] AS ''td'','''',[Source] AS ''td'','''',[Condition] AS ''td'','''',[Message] AS ''td'','''', [Occurence] AS ''td'','''' FROM  [DCS].dbo.Daily_Alarm_Events_UTIL_INSTRUMENTATION FOR XML PATH(''tr''), ELEMENTS) AS NVARCHAR(MAX))
SET @xml7 = CAST((SELECT [Priority] AS ''td'','''', [Timestamp] AS ''td'','''',[Source] AS ''td'','''',[Condition] AS ''td'','''',[Message] AS ''td'','''', [Occurence] AS ''td'','''' FROM  [DCS].dbo.Daily_Alarm_Events_AMMONIA_INSTRUMENTATION FOR XML PATH(''tr''), ELEMENTS) AS NVARCHAR(MAX))
SET @xml8 = CAST((SELECT [Priority] AS ''td'','''', [Timestamp] AS ''td'','''',[Source] AS ''td'','''',[Condition] AS ''td'','''',[Message] AS ''td'','''', [Occurence] AS ''td'','''' FROM  [DCS].dbo.Daily_Alarm_Events_PORT_INSTRUMENTATION FOR XML PATH(''tr''), ELEMENTS) AS NVARCHAR(MAX))
...
SET @emailIntro = N''<html><body><H1>Alarm Hit List For EIC</H1>'' +  
	...
	<p>Dear All,</p> 
	<p>Please find below the TOP 10 Alarm Hit List For Instrumentation Team.</p>
...
EXEC msdb.dbo.sp_send_dbmail @recipients=''youremail_1_@yourdomain.com;youremail_2_@yourdomain.com;youremail_X_@yourdomain.com'',
	@profile_name = ''PI'',
    @subject = ''Alarm Hit List For EIC'',  
    @body = @XBodyfinal,  
    @body_format = ''HTML'' ;
    ')
END
